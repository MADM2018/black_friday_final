...
El paquete "tidyverse" es el que usaremos para visualizar y explorar nuestro conjunto de datos. Es conocido por su sintaxis fácil de leer y su gran cantidad de funciones útiles. El paquete "scales" se utilizará principalmente para personalizar el eje del gráfico. 
También, el paquete "arules"" se utilizará en la parte final del análisis, Association Rule Learning y Apriori.
"Caret"" utilizamos para los modelos de regresión y "dplyr"" para el tratamiento de datos.
...
### Verificación de Valores No Determinados (NA).
...
### Eliminación de Valores No Determinados (NA).
...
### División de la Muestra para la Validación Cruzada --> Validation Set Approach.
...
#### Estimación de la cantidad de grupos posibles por la regla del codo.
...
labs(x = 'Categoria Ciudad', y = 'Total Compras (Dólares)', title = "Black Friday", #name title and axis
     subtitle = "Total Clientes en cada cluster por ciudad") + #name subtitle
...
theme(legend.box.background	= element_rect(colour = "black"),
      ...
      plot.title = element_text(hjust = 0, face = 'bold',color = 'black'), #title settings
      plot.subtitle = element_text(face = "italic")) + #subtitle settings
labs(x = 'Género', y = 'Total Compras (Dólares)', title = "Black Friday", #name title and axis
     subtitle = "Total Clientes en cada cluster por ciudad") + #name subtitle
guides(fill=guide_legend(title = "Cluster")) + #remove color legend
scale_y_continuous(labels = scales::comma) #prevent scientific number in x-axis
```
...
theme(legend.box.background	= element_rect(colour = "black"),
      ...
      plot.title = element_text(hjust = 0, face = 'bold',color = 'black'), #title settings
      plot.subtitle = element_text(face = "italic")) + #subtitle settings
labs(x = 'Rango Edad', y = 'Total Compras (Dólares)', title = "Black Friday", #name title and axis
     subtitle = "Total Clientes en cada cluster por ciudad") + #name subtitle
guides(fill=guide_legend(title = "Cluster")) + #remove color legend
scale_y_continuous(labels = scales::comma) #prevent scientific number in x-axis
```
...
theme(legend.box.background	= element_rect(colour = "black"),
      ...
      plot.title = element_text(hjust = 0, face = 'bold',color = 'black'), #title settings
      plot.subtitle = element_text(face = "italic")) + #subtitle settings
labs(x = 'Categoria Ciudad', y = 'Total Compras (Dolares)', title = "Black Friday", #name title and axis
     subtitle = "Total Clientes en cada cluster por ciudad") + #name subtitle
guides(fill=guide_legend(title = "Cluster")) + #remove color legend
scale_y_continuous(labels = scales::comma) #prevent scientific number in x-axis
```
...
theme(legend.box.background	= element_rect(colour = "black"),
      ...
      plot.title = element_text(hjust = 0, face = 'bold',color = 'black'), #title settings
      plot.subtitle = element_text(face = "italic")) + #subtitle settings
labs(x = 'Género', y = 'Total Compras (Dolares)', title = "Black Friday", #name title and axis
     subtitle = "Total Clientes en cada cluster por ciudad") + #name subtitle
guides(fill=guide_legend(title = "Cluster")) + #remove color legend
scale_y_continuous(labels = scales::comma) #prevent scientific number in x-axis
```
...
theme(legend.box.background	= element_rect(colour = "black"),
      ...
      plot.title = element_text(hjust = 0, face = 'bold',color = 'black'), #title settings
      plot.subtitle = element_text(face = "italic")) + #subtitle settings
labs(x = 'Rango Edad', y = 'Total Compras (Dolares)', title = "Black Friday", #name title and axis
     subtitle = "Total Clientes en cada cluster por ciudad") + #name subtitle
guides(fill=guide_legend(title = "Cluster")) + #remove color legend
scale_y_continuous(labels = scales::comma) #prevent scientific number in x-axis
```
...
El modelo final lo ejecutamos con todos los datos:
...
## Grafo Bipartito Producto-Cliente con los 20 productos mas vendidos

### Para poder renderizar el grafo tuvimos que dejarlo solamente con 50 aristas
```{r}
top_sellers = datos %>%
              count(Product_ID, sort = TRUE)
top_s = head(top_sellers,20)

datos_top = datos[datos$Product_ID %in% top_s$Product_ID, ]
datos_top = datos_top[1:30, ]

datos_grafo <- data.frame("from" = datos_top$User_ID, "to" = datos_top$Product_ID)
graph <- graph_from_data_frame(d=datos_grafo, directed=FALSE)

plot(graph)

```

### Para realizar algunos experimentos con el grafo vamos a utilizar el grafo completo, con todas sus aristas de los 20 productos mas vendidos
```{r}
datos_top = datos[datos$Product_ID %in% top_s$Product_ID, ]
...
### Vamos a comprobar que el grafo es bipartito
```{r}
ga_bi <- bipartite.mapping(graph)
```
El resultado nos queda en la variable res de ga_bi y en efecto es `r ga_bi$res`.

### Algunas propiedades interesantes del grafo que nos pueden ayudar a entender mejor el dataset
``` {r}
ga_order <- gorder(graph)
ga_size <- gsize(graph)
ga_dens <- round(edge_density(graph),4)
ga_cc <- components(graph)$no
ga_cc_size_lg = components(graph)$csize[1]
ga_avgdeg <- round(mean(degree(graph)), 0)
ga_diam <- diameter(graph)
ga_avgdist <- round(mean_distance(graph), 0)
ga_avg_clust_coef <- transitivity(graph, type="average")
```

Aqui algunas propiedades del grafo
...
* Número de componentes conexas: `r ga_cc` 

* Tamaño del la mayor componente conexa: `r ga_cc_size_lg`
...
* Coeficiente de clustering Promedio: `r ga_avg_clust_coef`

```{r}
ga_deg_dist = degree_distribution(graph)
plot(1:length(ga_deg_dist), ga_deg_dist, main="Scatter plot of degrees distribution", xlab="Degree", ylab="Distribution", pch=19)
```

### Ahora investigaremos acerca de las centralidades, con esto nos podemos hacer una idea de que producto es el mas deseado por los usuarios.
```{r}
degree_centr = attr(sort(degree(graph, normalized=TRUE), decreasing=TRUE), "name")[1:3]
betw_centr = attr(sort(betweenness(graph, normalized=TRUE), decreasing=TRUE), "name")[1:3]
eige_centr = attr(sort(eigen_centrality(graph, scale=FALSE)$vector, decreasing=TRUE), "name")[1:3]
```
...
El nodo **P00265242** que corresponde a un producto es el que mas se repiten en los tipos de centralidades y coincide con el producto con mas ventas. Podemos asumir que este producto es el mas deseado por los usuarios.
